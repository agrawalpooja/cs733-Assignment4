...
func Test_start(t *testing.T){
	for i:=1;i<=5;i++{
			os.RemoveAll("mylog"+strconv.Itoa(i))
	}
...
func TestRPCMain(t *testing.T) {
	...
	RaftNodes,_ := raft.Start()
	time.Sleep(time.Second*1)
	addr = []string{"localhost:8085","localhost:8086","localhost:8087","localhost:8088","localhost:8089"}
	for i:=0; i<5; i++{
		serverMain(RaftNodes[i],addr[i])
	}
}
...
func TestRPC_Redirect(t *testing.T) {
	...
	// Sending write/append command to follower and it will give redirection error with leader Id
	for i:=1; i<=5; i++{
		if i!=LId{
			cl = mkClient(t,i)
			break
	...
	m, err := cl.write("cs733file", data, 0)
	expect(t, m, &Msg{Kind: 'R'}, "redirection", err)
	cl.close()

	// Again tried with provided redirected node-id and got success 
	cl = mkClient(t,m.RedirectNode)
	defer cl.close()
	m, err = cl.write("cs733file", data, 0)
	expect(t, m, &Msg{Kind: 'O'}, "write success", err)
}

func TestRPC_KillLeaderServer(t *testing.T){
	LId := raft.GetLeaderId()
	cl := mkClient(t,LId)
	data := "abcbac"
	m, err := cl.write("abc", data, 0)
	expect(t, m, &Msg{Kind: 'O'}, "write success", err)
	cl.close()

	// Current Leader is killed : Now 4 nodes are up
	raft.KillLeader()
	time.Sleep(1500*time.Millisecond)

	newLId := raft.GetLeaderId()
	if newLId == LId{
		t.Fatal("Even in the availability of majority no new leader elected")
	}
	cl1 := mkClient(t,newLId)
	m, err = cl1.read("abc")
	expect(t, m, &Msg{Kind: 'C', Contents: []byte(data)}, "read my write after one node down", err)
	cl1.close()
	

	// Again current leader is killed : Now 3 nodes are up
	raft.KillLeader()
	time.Sleep(1500*time.Millisecond)

	nxtNewLId := raft.GetLeaderId()
	if nxtNewLId == newLId{
		t.Fatal("Even in the availability of majority no new leader elected")
	}
	cl2 := mkClient(t,nxtNewLId)
	m, err = cl2.read("abc")
	expect(t, m, &Msg{Kind: 'C', Contents: []byte(data)}, "read my write after two node down", err)
	cl2.close()
}
...
func mkClient(t *testing.T, RaftNodeId int) *Client {

	var client *Client
	raddr, err := net.ResolveTCPAddr("tcp", addr[RaftNodeId-1])
	if err == nil {
